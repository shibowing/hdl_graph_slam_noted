...
//订阅GPS信息
  ...
  /**
   * @brief
   * @return  添加GPS约束到pose_graph
   *   step1:将GPS经纬度先转换到UTM坐标系
   */
  bool flush_gps_queue() {
    ...
    for(const auto& gps_msg : gps_queue) {
      ...
      //每一个关键帧实际对应一帧GPS的位姿信息
      (*seek)->utm_coord = xyz;

      //这里我看gps的信息矩阵是常值，同时gps_edge_stddev基本都是1000，或者100
      Eigen::Matrix2d information_matrix = Eigen::Matrix2d::Identity() / gps_edge_stddev;
      graph_slam->add_se3_prior_xy_edge((*seek)->node, xyz.head<2>(), information_matrix); //这里只对xy 进行约束，而且约束的信息矩阵还比较小只有1/100或1/1000
